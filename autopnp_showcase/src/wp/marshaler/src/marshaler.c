...
/**
 * \brief  Performs marshaling for topic 'image'.
 *
...
)
{
	...
	// uint32_t topicData.height
	{
		uint32_t netValue;
		
		netValue = xme_hal_net_htonl((uint32_t)topicData.height);
		xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint32_t));
		bufferPtr += sizeof(uint32_t);
	}
	
	// uint32_t topicData.width
	{
		uint32_t netValue;
		
		netValue = xme_hal_net_htonl((uint32_t)topicData.width);
		xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint32_t));
		bufferPtr += sizeof(uint32_t);
	}
	
	// char topicData.encoding
	{
		uint8_t i0;
		
		for (i0 = 0; i0 < 20; i0++)
		{
			// char topicData.encoding[i0]
			xme_hal_mem_copy(bufferPtr, &topicData.encoding[i0], sizeof(char));
			bufferPtr += sizeof(char);
		}
	}
	
	// uint8_t topicData.isBigEndian
	{
		uint8_t netValue;
		
		netValue = (uint8_t)topicData.isBigEndian;
		xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint8_t));
		bufferPtr += sizeof(uint8_t);
	}
	
	// uint32_t topicData.step
	{
		uint32_t netValue;
		
		netValue = xme_hal_net_htonl((uint32_t)topicData.step);
		xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint32_t));
		bufferPtr += sizeof(uint32_t);
	}
	
	// uint8_t topicData.data
	{
		uint32_t i0;
		
		for (i0 = 0; i0 < 307200; i0++)
		{
			// uint8_t topicData.data[i0]
			{
				uint8_t netValue;
				
				netValue = (uint8_t)topicData.data[i0];
				xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint8_t));
				bufferPtr += sizeof(uint8_t);
			}
		}
